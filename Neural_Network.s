...
__main  FUNCTION	
       ...
       MOV R4,#5; // switch to label ; 1 - OR, 2 - AND, 3 - NOR, 4 - NAND, 5 - XNOR
       MOV R5,#1; 
       MOV R6,#2;  
	   MOV R7,#3;
	   MOV R8,#4;
	   ...
	   CMP R4,R5; 
	   BEQ OR;    // branch to label : OR  if switch = 1
       CMP R4,R6; 
	   BEQ AND;   // branch to label : AND if switch = 2
	   CMP R4,R7
	   BEQ NOR;   // branch to label : NOR  if switch = 3
	   CMP R4,R8; 
	   BEQ NAND;  // branch to label : NAND if switch = 4
	   CMP R4,R9;
	   BEQ XNOR;  // branch to label : NOR  if switch = 5
...
OR	   VLDR.F32 s14,=1; x1 = 1
	   ...
	   VADD.F32 s24,s24,s20; x1*w1 + x2*w2 + x3*w3 + bias = x 
	   B SIGM;
...
SIGM   VMOV.F32 s1,s24; // x = x1*w1 + x2*w2 + x3*w3 + bias
...
LOOPO  VMOV.F32 s7,s8;    // c1 = c2
	   ...
	   VCMP.F32 s3,s9;     // if count >= max no. of terms :- break from loop
